...
class BoidsLp:
    ...
    def lower_precision(self, x):
        return (x * (2 ** self.bits)).astype(int) / (2 ** self.bits)
    ...
    def update_boids(self, positions, velocities, return_accel=False):
        ...
        # Update positions
        positions = self.lower_precision(positions + self.lower_precision(velocities) * self.dt)
